...
def problem011(len_n):
    ...
    grid_init = """ 08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
    ...
    grid = [[int(i) for i in j] for j in [i.split() for i in grid_init]]
...
def problem012(n_div):
    ...
    addme = 1
    while True:
        if len(eul.return_divisors_list(tri)) > n_div:
            return tri
        else:
            tri = tri + addme
            addme += 1
...
def problem013(n):
    ...
    fiftydigits = """ 37107287533902102798797998220837590246510135740250
    ...
    data = [int(i) for i in fiftydigits.split()]
    return str(sum(data))[:n]
...
def problem014(limit):
    ...
    # Quite inefficient, takes some time.  Needs to be recoded.
